...
def visualize_rumor_propagation_realtime(network: SocialNetwork, origin: int, probability: float = 0.7,
    ...
    def update(frame):
        ...
        # Dessiner d'abord les arêtes normales (entre personnes non infectées)
        normal_edges = []
        infected_edges = []
        ...
        # Arêtes entre infectés en rouge
    ...
    print("   [Info] Les nœuds avec contour jaune sont les NOUVEAUX infectés de l'étape actuelle")
    print("   [Info] Les arêtes ROUGES relient les personnes infectées entre elles")
    ...
    print(f"\n" + "=" * 60)
    print("[RÉSUMÉ DÉTAILLÉ DE LA PROPAGATION]")
    print("=" * 60)
    ...
    # Calculer les arêtes entre infectés
...
def menu_find_cliques(network: SocialNetwork):
    """Menu interactif pour la détection de CERCLES D'AMIS COMPLETS."""
    print("\n" + "=" * 60)
...
def menu_rumor_propagation(network: SocialNetwork):
    ...
    print("\n[Lancement de l'animation en temps réel...]")
...
def analyze_graph(G: nx.Graph, groups: List[List[int]], group_activities: Dict[int, str]):
    ...
    print(f"\n[Top 5 des personnes les plus connectées]")
...
def interactive_menu():
    ...
    while True:
        print("\n" + "=" * 60)
        print("[SIMULATEUR DE RÉSEAU SOCIAL INTERACTIF]")
        print("=" * 60)
        print("\n[MENU PRINCIPAL]")
        ...
        if choice == "1":
            ...
            max_people = ask_positive_int("> Nombre MAX de personnes par sous-groupe (min: 5) : ", 5)

            print("\n[Génération du réseau en cours...]")
            print("   [Info] Entre 2 et 5 connexions aléatoires entre chaque paire de groupes")
            print("   [Info] Chaque groupe se voit assigner une activité unique")
            G, groups, group_activities = generate_social_graph(nb_groups, max_people)
            network = SocialNetwork(G, groups, group_activities)

            print("\n[OK] Réseau généré avec succès !")
